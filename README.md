...
**After a crash or a hard kill, a Drift entry can survive in an external CLI's configuration.** Drift removes its `drift` entry from `~/.gemini/settings.json` and `~/.codex/config.toml` when a session ends, but a Drift that is killed outright never runs that cleanup. The Codex entry carries your Caido session token in plain text (the Gemini entry carries a `${CAIDO_TOKEN}` reference, not the token bytes). The next Drift start sweeps both CLIs before it registers anything — every scope Drift has ever written, whether or not the provider is still enabled and whether or not this run ever registered — so the leftover entry is normally removed then. **The one case it cannot cover: Drift removes an entry by running that CLI's own `mcp remove`, so if the CLI's binary has been uninstalled, renamed or moved (or you have cleared its command field in Drift settings) there is nothing to run and the entry stays.** Drift says so in its log when that happens, with the exact commands to paste. To remove it by hand instead, run the same commands Drift prints if a removal fails: `gemini mcp remove --scope user drift`, `gemini mcp remove --scope project drift`, and `codex mcp remove drift`.
