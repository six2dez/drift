...
**After a crash or a hard kill, a Drift entry can survive in an external CLI's configuration.** Drift removes its `drift` entry from `~/.gemini/settings.json` and `~/.codex/config.toml` when a session ends, but a Drift that is killed outright never runs that cleanup. The Codex entry carries your Caido session token in plain text (the Gemini entry carries a `${CAIDO_TOKEN}` reference, not the token bytes). The next Drift start sweeps both CLIs before it registers anything — every scope Drift has ever written, whether or not the provider is still enabled and whether or not this run ever registered — so the leftover entry is normally removed then. **The sweep runs as part of starting the MCP server, so the cases it cannot cover are the ones where that never happens:** if you turn the MCP server off in Drift settings, if Drift has no Caido session token yet, or if its own startup checks fail, no start means no sweep. It also cannot cover a CLI it can no longer run — Drift removes an entry by running that CLI's own `mcp remove`, so if the binary has been uninstalled, renamed or moved (or you have cleared its command field in Drift settings) there is nothing to run and the entry stays. Drift says so in its log in that last case, with the exact commands to paste; in the others it has not run at all and so says nothing. To remove it by hand instead, run the same commands Drift prints if a removal fails: `gemini mcp remove --scope user drift`, `gemini mcp remove --scope project drift`, and `codex mcp remove drift`.
