...
// ── TEMPORARY: Phase 8 Wave-0 spike (A1) — DELETED BY T-08-03 ──────
//
// Wave-0 risk closure for assumption A1 of 08-RESEARCH.md. Caido's LLRT fork was
// SOURCE-READ (modules/llrt_child_process/src/lib.rs:512-521 calls
// `process_group(0)` when the spawn option is set) and never EXECUTED. Every
// vitest leg in this repository runs Node, so a runtime that quietly ignored that
// option would leave all five CI legs green while LIF-02 stayed unfixed on every
// real install. This block runs the experiment inside Caido's own backend and
// reports the verdict through the existing Copy-diagnostics action.
//
// It is temporary by construction. T-08-03 removes everything between this marker
// and the matching `end TEMPORARY` line, plus the single getDiagnostics spread.
// Nothing here is referenced from anywhere else in the file.
//
// Security (threat T-08-05, under the T-04-04 rule governing every other field in
// that record): the three fields contributed here are fixed literals and one
// `typeof` word. No pid, no path, no environment value and no error `message` is
// ever rendered — an error surfaces as its constructor name only.

// A LOCAL cast, deliberately NOT an edit to SpawnWithEnv (:2003). Plan 08-02 owns
// adding the option to the real alias as a REQUIRED member, which forces all three
// of its call sites to state an answer out loud; doing that here would make three
// unrelated sites answer a question before the tracer that justifies it exists.
type SpawnDetached = (
  command: string,
  args: string[],
  options: Record<"env", Record<string, string>> & {
...
const spawnDetached = spawn as unknown as SpawnDetached;

// The leak guard, and the reason this probe cannot strand a process: both fixture
// levels exit on their own after 30 s regardless of what the group kill does.
const SPIKE_GRANDCHILD_SOURCE = "setTimeout(function () {}, 30000);";

// The fixture parent stands in for a provider CLI: it forks one child — the
// stand-in for `node mcp-server.mjs`, the process that carries CAIDO_TOKEN on the
// production path — announces both pids on stdout, then idles. This reproduces
// case A of 08-RESEARCH.md § Measured experiment, where killing the parent alone
// leaves the grandchild running.
const SPIKE_FIXTURE_SOURCE =
  'const cp = require("child_process");' +
  `const g = cp.spawn(process.execPath, ["-e", ${JSON.stringify(SPIKE_GRANDCHILD_SOURCE)}], { stdio: "ignore" });` +
...
async function runLifecycleSpike(): Promise<Record<string, string>> {
  ...
  // A POSIX-only question. LLRT's win32 arm of the same branch sets
  // DETACHED_PROCESS, which `taskkill /T` does not consult at all — it walks the
  // parent-child relation — so there is nothing here for Windows to answer.
  if (host?.platform === "win32") {
    const na = "n/a (win32 — POSIX-only probe)";
    return {
      spikeProcessKillType: na,
      spikeDetachedGroupKill: na,
      spikeNote: na,
    };
  }

  // Reached through the same guarded globalThis cast readParentEnv (:563) uses and
  // for the same reason: @caido/quickjs-types declares no `process` global at all,
  // so a bare `process.` reference is a ReferenceError rather than an undefined.
  // Declared without an initializer on purpose: both arms below assign, so a
  // default here is an assignment no path ever reads (no-useless-assignment).
  let spikeProcessKillType: string;
  try {
    const killTypeRef = globalThis as typeof globalThis & {
      process?: { kill?: unknown };
    };
    spikeProcessKillType = typeof killTypeRef.process?.kill;
  } catch {
    spikeProcessKillType = "unavailable";
  }

  // The same guard again, narrowed to the callable shape Q5 verified. Kept
  // separate from the reader above so the reported `typeof` word is derived from
  // an unknown, never from a type this file asserted.
  const signalRef = globalThis as typeof globalThis & {
    process?: { kill?: (pid: number, signal?: string | number) => boolean };
  };

  const nodeExecutable = await getCachedNodeExecutable();
  if (nodeExecutable === undefined) {
    return {
      spikeProcessKillType,
      spikeDetachedGroupKill: "skipped (node executable not resolved)",
      spikeNote,
    };
  }
  ...
  let spikeDetachedGroupKill = "inconclusive (no fixture output)";
  ...
  try {
    ...
    const fixture = spawnDetached(nodeExecutable, ["-e", SPIKE_FIXTURE_SOURCE], {
      env: buildSpawnEnv({ parentEnv: readParentEnv(), driftVars: {} }),
      stdio: ["pipe", "pipe", "pipe"],
      windowsVerbatimArguments: false,
      detached: true,
    });
    // Deliberately NOT awaited to close. The fixture is long-lived, and awaiting a
    // long-lived child inside an RPC handler is exactly the starvation class
    // CLAUDE.md § Anti-Patterns names.
    fixture.stdout?.on("data", (d: Buffer) => {
      if (parentPid !== undefined) return;
      try {
        const parsed = JSON.parse(d.toString().split("\n")[0] ?? "") as {
          parent?: unknown;
          grandchild?: unknown;
        };
        if (typeof parsed.parent === "number") parentPid = parsed.parent;
        if (typeof parsed.grandchild === "number") grandchildPid = parsed.grandchild;
      } catch {
        // A truncated first chunk is indistinguishable from a fixture that never
        // printed. Both land on the "inconclusive" verdict below, which is a
        // non-answer and not a pass.
      }
    });
  } catch (e) {
    return {
      spikeProcessKillType,
      spikeDetachedGroupKill: `error: ${e instanceof Error ? e.constructor.name : typeof e}`,
      spikeNote,
    };
  }

  // The only wait primitive this file has evidence for inside an RPC handler:
  // resolveCommand and unregisterMcpFromCli both already yield this way. A bare
  // setTimeout promise does not have that evidence — CLAUDE.md § Anti-Patterns
  // records that timer callbacks do not fire while an outer await is suspended.
  await spawnAndWait(nodeExecutable, ["-e", "setTimeout(function () {}, 600);"]);

  if (parentPid !== undefined && grandchildPid !== undefined) {
    // The OS utility, NOT the runtime's own negative-pid signalling form.
    // 08-RESEARCH.md § Q2 proves the latter raises an Underflow conversion error
    // under LLRT's u32 pid parameter while passing under Node, so using it here
    // would make the probe report a false negative for the very assumption it
    // exists to close (threat T-08-06). Rendering the argv by hand is acceptable
    // ONLY because T-08-03 deletes it; plan 08-02 moves the rendering into
    // buildKillTreePlan where a test can reach it.
    await spawnAndWait("kill", ["-TERM", "--", `-${String(parentPid)}`]);
    await spawnAndWait(nodeExecutable, ["-e", "setTimeout(function () {}, 600);"]);
    try {
      // Signal 0 is a positive-pid call, so Q2's u32 range check does not apply.
      // An EPERM still throws on unix, hence the guard.
  ...
  // Threat T-08-09. Unconditional, on every path that captured a pid — including
  // the inconclusive and error arms — so a failed experiment leaks nothing.
  for (const pid of [parentPid, grandchildPid]) {
    if (pid === undefined) continue;
    try {
      signalRef.process?.kill?.(pid, "SIGKILL");
    } catch {
      // Already dead, or no longer ours. Either way there is nothing to do.
    }
  }
...
// ── end TEMPORARY Phase 8 Wave-0 spike ─────────────────────────────
...
async function getDiagnostics(_sdk: BackendSDK): Promise<Result<Record<string, string>>> {
  ...
  const info: Record<string, string> = {
    ...
    // TEMPORARY (Phase 8 Wave-0 spike, removed by T-08-03). Three fields, all of
    // them fixed literals or a `typeof` word — the record's T-04-04 rule holds.
    ...(await runLifecycleSpike()),
