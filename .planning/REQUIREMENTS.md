...
- [ ] **LIF-01**: On Windows, cancelling or timing out a turn terminates the whole process tree (`taskkill /pid <pid> /T /F`), leaving no orphaned token-bearing process *(unticked 2026-08-27 and STILL UNTICKED. **CURRENT 2026-08-31, Plan 08-23:** the safe native suite contains exactly two cases and both remain unexecuted because no `windows-latest`/native run exists. CI pins `expectedTotal = 2` plus the full behavioural test identity. Commit `e1ac837` removed the former already-exited/dead-pid exit-code and stderr measurement because a recycled pid could target an unrelated process; no reserved block exists. That missing datum is explicitly deferred and accepted until a safe owned-live-process measurement is designed. **The mechanism is BUILT and unit-proven, not natively executed.** Residuals **AR-01** (`taskkill /T` cannot reach a grandchild whose intermediate parent already exited) and **AR-04** (no Windows command-line enumerator) remain. **Owner of native execution: ROADMAP Phase 9 SC-4.** Nothing in this record correction closes the requirement.)*
...
<!-- DRIFT:WINDOWS-TWO-CASE:END -->
  > *SUPERSEDED 2026-08-31 by the Plan 08-23 correction above; preserved, not deleted:* "On Windows, cancelling or timing out a turn terminates the whole process tree (`taskkill /pid <pid> /T /F`), leaving no orphaned token-bearing process (unticked 2026-08-27 and STILL UNTICKED; prose corrected 2026-08-28 by plan 08-13. The mechanism is BUILT and unit-proven. What has NO execution behind it is the verb terminates: `kill-tree.win32.test.ts` is `skipIf`-gated to win32 and is 3/3 pending on every host that has ever run it, and no `windows-latest` run has ever executed it — broken-windows ledger entry 12. The AR-01 and AR-04 residuals remain; Phase 9 SC-4 owns the Windows run.)"
  > *SUPERSEDED (written 2026-08-27):* On Windows, cancelling or timing out a turn terminates the whole process tree (`taskkill /pid <pid> /T /F`), leaving no orphaned token-bearing process *(unticked 2026-08-27 to agree with `08-VERIFICATION.md`'s `human_needed` verdict. **The mechanism is BUILT and unit-proven** — `buildKillTreePlan`'s win32 arm emits `<SystemRoot>\System32\taskkill.exe /pid <n> /t /f` by absolute path, with a derived-system-root rung added by plan 08-08, all green in `kill-plan.test.ts`. **What has NO execution behind it is the verb "terminates":** `kill-tree.win32.test.ts` is `skipIf`-gated to win32 and is 3/3 pending on every host that has ever run it, and **no `windows-latest` run has ever executed it** — broken-windows ledger entry 12. Closes on one green Windows leg.)*
- [ ] **LIF-02**: On POSIX, cancelling or timing out a turn also terminates the CLI's MCP child (which carries `CAIDO_TOKEN`), via process-group signalling rather than a single-pid signal *(added 2026-08-12; unticked 2026-08-27 and STILL UNTICKED; prose corrected 2026-08-28 by plan 08-13 to cite `08-VERIFICATION.md`'s `gaps_found` verdict, which superseded the `human_needed` verdict this row cited until today. **The mechanism is BUILT and behaviourally proven under Node** — `kill-tree.posix.test.ts` runs a real control-plus-proof pair. **The clause that recorded assumption A1's verdict as favourable on real hardware on 2026-08-27 is RETRACTED** — its exact superseded wording is preserved in the block quote beneath this row rather than restated on a live line, because no line-oriented scan can tell a quoted phrase from an assertion. The probe determined liveness with `signalRef.process?.kill?.(pid, 0) ?? false` on a runtime the SAME diagnostics run measured as having no `process.kill`, so the optional chain yielded `undefined`, `?? false` made `alive === false`, and the favourable string was emitted UNCONDITIONALLY; the red input did not exist, so the reading carries no information and **A1 is OPEN, not measured**. **But assumption A6 was measured FALSE** on 2026-08-27: codex pid 43921 sat in pgid 43752 while its own `mcp-server.mjs` child pid 44284 sat in pgid 44284, so for that provider the group signal does **not** reach the token-bearing child, and the requirement's stated mechanism does not close it — that measurement is NOT withdrawn. What closes it independently of process groups is the **argv-marker orphan reap** shipped by plans 08-06/08-07 — and that reap is itself covered by no executed assertion on the shipping runtime, ledger entries 13 and 15. Claude Code, the active provider, remains unmeasured. **UPDATED 2026-08-31 by plan 08-17, and the row STAYS UNTICKED — the update moves evidence in both directions and the unfavourable direction is the one that governs.** Three things changed. (1) **A1's causal half was re-run on the patched three-valued probe and came back favourably**, from an instrument that could have printed either of the other two values; ledger entry 20 is closed on it. **The 2026-08-27 reading is NOT reinstated and stays RETRACTED** — a later measurement on a repaired instrument does not make an earlier vacuous one informative — and the superseded wording beneath this row is untouched. (2) **A6 was measured TRUE for Claude Code** (plan 08-16), so A6 is a per-provider split rather than a reversal; the codex FALSE reading stands unchanged. (3) **The pre-fix Control was taken and CONTRADICTS its own prediction:** on build `68199fa` — no `detached` at the provider spawn, no reap, only a single-pid SIGTERM — the after-Stop count was **ZERO**, where `08-01` truth 3 predicted non-zero. **So this requirement's own stated mechanism, "via process-group signalling", is the one thing still not shown to be doing the work:** Claude Code cleans up its own MCP child on SIGTERM, which makes the group kill REDUNDANT on the one combination measured, and nothing in this repository yet demonstrates a case where Drift's group signal is what prevented an orphan. Redundant is not useless — codex is FALSIFIED for A6 and untested on the Control path, and it is the provider the one real orphan report came from — but it is not "closed" either. **What still blocks this row:** ledger entries 13 and 15 (the reap has no executed assertion, both narrowed 2026-08-31 and both still open); the Control unrun against any non-Claude provider; and the redundancy question itself, recorded as ownerless in `deferred-items.md` items 3 and 11. **No later phase in this milestone owns any of them.**)*
  > *SUPERSEDED (written 2026-08-27; the A1 clause was RETRACTED 2026-08-28):* On POSIX, cancelling or timing out a turn also terminates the CLI's MCP child (which carries `CAIDO_TOKEN`), via process-group signalling rather than a single-pid signal *(added 2026-08-12; unticked 2026-08-27 to agree with `08-VERIFICATION.md`'s `human_needed` verdict. **The mechanism is BUILT and behaviourally proven under Node** — `kill-tree.posix.test.ts` runs a real control-plus-proof pair — and assumption A1 was measured favourably on real hardware on 2026-08-27. **But assumption A6 was measured FALSE** on the same day: codex pid 43921 sat in pgid 43752 while its own `mcp-server.mjs` child pid 44284 sat in pgid 44284, so for that provider the group signal does **not** reach the token-bearing child, and the requirement's stated mechanism does not close it. What closes it independently of process groups is the **argv-marker orphan reap** shipped by plans 08-06/08-07 — and that reap is itself covered by no executed assertion, ledger entries 13 and 15. Claude Code, the active provider, remains unmeasured.)*
...
| LIF-01 | Phase 8 | Partial — mechanism built, Windows execution unrun. Verdict `gaps_found` (`08-VERIFICATION.md`; supersedes the `human_needed` verdict this row cited until 2026-08-28). Bounded by **AR-01** (`/T` cannot reach a grandchild whose intermediate parent exited) and **AR-04** (no Windows enumerator). **Not closing in Phase 8. Owner: Phase 9 SC-4** — a green `windows-latest` leg |
| LIF-02 | Phase 8 | Partial — Plan 08-18 closed deferred item 10 and propagated the repaired A1 result across exactly eight mutable A1 carriers plus two live pointers while preserving the invalid 2026-08-27 reading as retracted history. **STILL NOT CLOSING IN PHASE 8:** the argv-marker reap lacks an executed assertion on the shipping runtime (entries 13 and 15), no Control exists for a non-Claude provider, and the redundancy question remains unresolved (deferred items 3 and 11). LIF-01/LIF-02 remain unchecked; no record repair supplies runtime causality. Verdict `gaps_found` (`08-VERIFICATION.md`). |
...
<!-- DRIFT:LIF-02-TRACEABILITY:BEGIN -->
...
<!-- DRIFT:LIF-02-TRACEABILITY:END -->

> **SUPERSEDED 2026-08-31 by the Plan 08-23 correction above — pre-08-18 traceability row preserved verbatim:** `| LIF-02 | Phase 8 | Partial — mechanism built; A6 is a per-provider split (TRUE Claude Code / FALSE codex); reap still unasserted on the shipping runtime (entries 13, 15); **A1's 2026-08-27 reading stays RETRACTED** and its causal half was **re-measured on 2026-08-31** on the patched probe, with a result recorded in `08-SPIKE.md` Table 1 (entry 20 closed). **That verdict is deliberately NOT restated here on a live line** — until the propagation follow-up (`deferred-items.md` item 10) updates all eight A1 carriers together, `08-SPIKE.md` is the single carrier of the new verdict and this row points at it rather than becoming a second one. Verdict `gaps_found` (`08-VERIFICATION.md`). **STILL NOT CLOSING IN PHASE 8, and the reason changed on 2026-08-31:** the pre-fix Control was taken and came back ZERO, so the CLI-cleanup confounder is **implicated rather than excluded** and this requirement's stated process-group mechanism is not shown to be what does the work on the one combination measured. **Owners:** the Windows execution leg is **Phase 9 SC-4**. **Everything else is OWNERLESS** — the reap assertion, a Control against a non-Claude provider, and the redundancy question — recorded in `deferred-items.md` items 3, 4, 5 and 11. The A1 re-run itself is DONE (plan 08-17) |`
