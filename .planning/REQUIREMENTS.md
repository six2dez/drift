...
- [x] **LIF-01**: On Windows, cancelling or timing out a turn terminates the owned live process tree with `taskkill /pid <pid> /T /F`, leaving no tracked token-bearing descendant. *(Closed 2026-09-02: native CI run `33599694679` on candidate `9aa22bb` executed the exact safe suite 2 passed / 0 pending / 0 failed, killed a live parent and grandchild through the production argv, and resolved the killer from the runner system root. AR-01 and AR-04 remain explicit accepted residuals.)*
...
- [x] **LIF-02**: On POSIX, cancelling or timing out a turn leaves no token-bearing MCP child: process-group termination covers children that remain in the provider group, and the session-unique argv-marker reap covers children that leave it. *(Closed 2026-09-02: the installed candidate in real Caido reaped a separately grouped marker fixture with `kind=reap exit=0 attempted=1 ageMs=109`; the target died before the token-bearing runtime root was removed. The completion-order harness independently proves all teardown prerequisites settle before recursive removal.)*
...
| LIF-01 | Phase 8 | Complete — native Windows CI run `33599694679` executed the exact safe contract 2/2, including live parent/grandchild termination and absolute system-root `taskkill.exe` resolution. AR-01 and AR-04 remain accepted residual boundaries rather than hidden exceptions. Verdict `passed` (`08-VERIFICATION-6.md`). |
| LIF-02 | Phase 8 | Complete — real-Caido execution forced the session-marker reap (`kind=reap exit=0 attempted=1 ageMs=109`) against a separately grouped fixture, and the completion-order harness proves kill/reap settlement precedes recursive token-root removal. The requirement follows ROADMAP SC-2's two-mechanism contract, not the superseded process-group-only wording. Verdict `passed` (`08-VERIFICATION-6.md`). |
