...
| LIF-02 | Phase 8 | Partial — mechanism built, A6 measured FALSE, reap unasserted on the shipping runtime, **A1 RETRACTED 2026-08-28**. Verdict `gaps_found` (`08-VERIFICATION.md`; supersedes the `human_needed` verdict this row cited until 2026-08-28). **Not closing in Phase 8 — owners:** the Windows execution leg is **Phase 9 SC-4**; the post-fix POSIX cancel and timeout measurements are **Phase 8 plan 08-16**; the A1 re-run with the fixed probe is **Phase 8 plan 08-17**. **After Phase 8 closes, NO later phase in this milestone owns the A1 re-run** — stated plainly because it is the fact most likely to be lost |
