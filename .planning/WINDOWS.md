...
fixed_count: 12
total_count: 24
last_updated: 2026-08-31T16:44:28.000Z
---
...
| 11 | 08 | unmet-truth | packages/backend/src/kill-plan.ts |  | MEASURED 2026-08-27 on a real macOS Caido (darwin 25.6.0, probe build 68199fa), superseding the 2026-08-24 entry; CORRECTED 2026-08-28, superseding the A1 half of that 2026-08-27 rewrite. Both superseded texts are preserved verbatim in the marked-correction notes at the end of this file. A1 IS RETRACTED, NOT NARROWED: spikeDetachedGroupKill decided liveness with an optional call on process.kill, a primitive the SAME diagnostics run measured ABSENT (spikeProcessKillType: 'undefined'), so the optional chain yielded undefined, '?? false' forced alive === false, and the reading 'grandchild-died (detached honoured)' was emitted UNCONDITIONALLY. The red input did not exist, so that reading carries no information about whether the grandchild died. CONSEQUENCE FOR THIS ENTRY, STATED PLAINLY RATHER THAN LEFT IN A MIDDLE STATE: the nine-site POSIX regression this entry was opened for is LIVE AGAIN. Whether the shipped LLRT honours the detached spawn is UNMEASURED, and the nine POSIX termination sites rest on unexecuted source analysis exactly as they did on 2026-08-24. A6 is FALSIFIED: codex pid 43921 sat in pgid 43752 while its own mcp-server.mjs child pid 44284 sat in pgid 44284, so a group signal aimed at the CLI's group cannot reach the token-bearing child, and OQ-2's single-pid rung does not help because it signals the CLI rather than the child. That reading was taken by direct ps observation, is NOT withdrawn, and is NOT bundled into the A1 retraction. The phase's answer to the falsification is the argv-marker orphan reap shipped by plans 08-06 and 08-07 - pgrep -f over a session-unique temp-dir marker adjacent to mcp-server.mjs, then a POSITIVE single-pid kill - which identifies its target by the target's own command line and is therefore independent of process groups by construction. NARROWED 2026-08-31 by plan 08-17 and NOT CLOSED. Three of this entry's four open clauses are now answered and the fourth is answered in the WRONG DIRECTION, which is why the entry survives. (a) A1's causal half is MEASURED and favourable - the 2026-08-31 re-run on the patched three-valued probe returned 'grandchild-died (detached honoured)' from an instrument that could have printed either of the other two values; see entry 20, now closed. (b) A1's topology half is measured as a genuine CONTROL PAIR on one machine minutes apart: post-fix the Drift-spawned provider carries pgid == pid (3386/3386), pre-fix on build 68199fa it inherits Caido's own group 91048, twice (5190 and 7005). One variable, the build. So Phase 8's detached: true is what creates the group and the shipping LLRT honours it. (c) A6 is measured for Claude Code (plan 08-16 Table 4, TRUE) and stays FALSIFIED for codex, so A6 is a per-provider SPLIT rather than a reversal. (d) THE CONTROL WAS TAKEN AND CONTRADICTS ITS OWN PREDICTION, and this is what keeps the entry open: 08-01 truth 3 predicted a NON-ZERO pgrep -f mcp-server.mjs count after Stop on the pre-fix build; the measurement is ZERO (1 during the turn at 10:55:54, first zero at 10:56:05, six consecutive zero samples, Stop corroborated by exitCode 143 = SIGTERM in the session record). On a build with no detached at the provider spawn and only a single-pid SIGTERM, the token-bearing MCP child died anyway. The CLI-cleanup confounder is therefore no longer merely unexcluded - IT IS IMPLICATED - and plan 08-16's post-fix zero proves LESS than it appeared to, because the pre-fix build yields the same zero. For Claude Code on macOS, Drift's group-kill machinery is REDUNDANT with the provider's own cleanup on this path. It is NOT shown useless: the 2026-08-27 G-04 orphan was a codex process, foreign-parented, that Drift never spawned, and A6 shows codex puts its MCP child in its OWN process group, so neither the group kill nor this cleanup path is established for it. WHAT KEEPS THIS ENTRY OPEN: the reap is still covered by no executed assertion (entries 13 and 15, both narrowed and both still open); the redundancy finding in (d) is a NEW open question about what the nine POSIX termination sites are actually buying on this one combination, and it was created by this reading rather than closed by it; and every reading above is one provider (claude-cli), one platform (darwin 25.6.0), one Caido version (0.58.2), one machine, with codex, gemini, copilot, Linux and Windows all unmeasured. Closes on an executed assertion over the reap wiring plus at least one non-Claude provider measured on the Control path. | open | A1 RETRACTED 2026-08-28: the 2026-08-27 reading was emitted unconditionally by a probe whose liveness primitive the same run measured absent, so it narrows nothing and the nine-site POSIX concern this entry was opened for is live again. A6 measured FALSE 2026-08-27 by direct ps observation, for one provider on one build; Claude Code unmeasured and the pre-fix Control never taken. | 2026-08-24T15:24:21.284Z |  |
| 12 | 08 | unrun-verify | packages/backend/src/kill-tree.win32.test.ts |  | Exactly two native win32 kill-tree cases form the 2/2 contract. CI run 33599694679 on candidate 9aa22bbd4c26fecabc17745438a2217a24b31d3e executed 2 passed, 0 pending and 0 failed; the behavioural case killed the live parent and grandchild, and the resolution case used the absolute system-root taskkill.exe. Commit e1ac837 removed the unsafe already-exited/dead-pid exit-code and stderr vehicle; no reserved block exists and that datum remains accepted and deferred. LIF-01 is closed at the owned live-tree contract; AR-01 and AR-04 remain accepted residuals. | fixed | CI https://github.com/six2dez/drift/actions/runs/33599694679 | 2026-08-24T15:54:03.743Z | 2026-09-02T06:39:13Z |
| 13 | 08 | unrun-verify | packages/backend/src/index.ts |  | reapMcpOrphans and its cleanupMcpRuntime call site are covered by no executed assertion — index.ts cannot be imported under vitest and no static source gate was added (plan rated the truth 'verification: backstop'). NARROWED 2026-08-28 by plan 08-16 and NOT CLOSED; the superseded text is preserved verbatim in the marked-correction note at the end of this file. A real cancel on the maintainer's macOS Caido 0.58.2 (darwin 25.6.0, build 39876b5) produced lastOrphanReap = 'kind=noop reason=scan-failed exit=1 killed=0 ageMs=24'. exit=1 is reachable only through scanner.on('close', code) — both spawnThrew paths and the timeout arm pass exitCode undefined, which formatOrphanReapRecord renders as exit=undefined — so the value proves the cleanupMcpRuntime call site EXECUTED on the shipping runtime and wrote its record. NARROWED AGAIN 2026-08-31 by plan 08-17 and STILL NOT CLOSED. The HEAD build at 10:47 reported lastOrphanReap = 'kind=reap exit=0 killed=2 ageMs=31' with activeSessions 0. This is the FIRST observation anywhere of the reap taking its kind=reap arm and actually signalling pids; every prior reading was kind=noop, so until now the mechanism had only ever been observed matching nothing. Three days had elapsed since the 2026-08-28 session, so a start-up previous-run reap (AR-02 / OQ-3) clearing orphans that session left behind is CONSISTENT with the value. WHAT THE VALUE DOES NOT ESTABLISH: what it killed. The record carries a count, not identities, so its targets are deliberately not named. WHAT KEEPS THIS ENTRY OPEN, unchanged by the observation: a runtime observation is not an executed assertion. No test asserts this wiring, index.ts is still not importable under vitest, no static source gate was added, and the readings cover ONE platform and TWO builds. An entry narrowed is an entry still open. | open |  | 2026-08-27T12:08:50.726Z |  |
...
| 15 | 08 | unrun-verify | packages/backend/src/index.ts |  | reapMcpOrphans / reapSessionOrphansIfIdle wiring is asserted only by source text. NARROWED 2026-08-28 by plan 08-16 and NOT CLOSED; the superseded text is preserved verbatim in the marked-correction note at the end of this file. The SECOND half of the original entry — whether Caido's LLRT sandbox can spawn pgrep at all — is now MEASURED and answered YES, by bare name, on the maintainer's macOS Caido 0.58.2 (build 39876b5); see entry 14 for the reading and its derivation. Neither reap therefore degrades silently to the enumerator-unavailable no-op on that platform. NARROWED AGAIN 2026-08-31 by plan 08-17 and STILL NOT CLOSED. The 2026-08-31 HEAD-build reading 'kind=reap exit=0 killed=2 ageMs=31' shows the reap reaching its signalling arm rather than only its no-match arm, so the mechanism is demonstrably capable of acting and not merely of running. WHAT KEEPS THIS ENTRY OPEN, unchanged by either reading: the wiring is still asserted only by source text, index.ts is still not importable under vitest, and the IDLE-GATED session reap reapSessionOrphansIfIdle was NOT exercised by the 2026-08-28 or the 2026-08-31 reading - both came from paths other than that call site, and the 2026-08-31 value is consistent with the START-UP previous-run reap, which is a third call site again. activeSessions was 0 in every capture, so the idle gate's interesting case (activeSessions greater than 0 with a Drift-owned MCP call in flight) has still never been observed. An entry narrowed is an entry still open. | open |  | 2026-08-27T12:30:13.421Z |  |
...
| 20 | 08 | deviation | packages/backend/src/index.ts |  | Fourteenth vacuous gate in Phase 8, and the first that shipped as a READING rather than as a green tick. The A1 probe at the recoverable commit 68199fa (index.ts:5061) decided the verdict with 'const alive = signalRef.process?.kill?.(grandchildPid, 0) ?? false', an optional call on a primitive the SAME diagnostics run measured ABSENT five lines earlier ('spikeProcessKillType: undefined'); the absent case was coalesced to 'not alive', so the favourable string 'grandchild-died (detached honoured)' was emitted UNCONDITIONALLY. THE RED INPUT DID NOT EXIST. WHAT MAKES THIS INSTANCE DIFFERENT FROM THE TWELVE ALREADY RECORDED (entries 16, 17, 18, 19 and the self-reports in this phase's SUMMARYs): those produced a green tick, which a reviewer distrusts by habit; this one produced a plausible-looking hardware READING, which reviewers propagate. It propagated into eleven carriers, .planning/ROADMAP.md, .planning/REQUIREMENTS.md, four phase documents, three source files and this ledger's own entry 11, and was pinned there by verdict-gate.sh ARM B before anybody re-derived it. FIXED IN CODE: classifyLivenessObservation in packages/backend/src/kill-plan.ts (plan 08-12) is three-valued - alive / dead / inconclusive - and reports inconclusive when the liveness primitive is unavailable instead of coalescing it into a verdict. CLOSED 2026-08-31 by plan 08-17 on the exact reading this entry named as its closing condition. The A1 re-run was taken on the maintainer's macOS Caido 0.58.2 (darwin 25.6.0, provider claude-cli) on the probe build 68199fa plus a1-probe-fix.patch, plugin id d8aee773-b939-4164-a576-9c276ee30df8. spikeDetachedGroupKill read 'grandchild-died (detached honoured)', which is formatSpikeVerdict's dead arm and one of exactly three permitted values. THE INSTRUMENT COULD HAVE SAID OTHERWISE, which is the property the withdrawn reading lacked: classifyLivenessObservation DEFAULTS to inconclusive and must clear five gates before it can return dead (a spawned enumerator, no timeout, a usable pid, a numeric exit code that is neither undefined nor null, and that code being one of two recognised values), and the patch passes spawnThrew: !probe.spawned specifically so that spawnAndWait's SYNTHETIC code 1 on an unspawnable enumerator - byte-identical to ps's documented no-process-matched - cannot become the answer that the grandchild is gone. That is gap 1 rebuilt out of a different operator, and the patch refuses it by name. NOTE THAT spikeProcessKillType STILL READ 'undefined' in the same dump, reproducing the retracted reading's exact surface signature; what makes this one a measurement is that THE CODE BETWEEN THE TWO VALUES IS DIFFERENT, and the fixture check pgrep -f 'node -e' returning 0 immediately after the diagnostics call proves the PATCHED build is what ran, because the original's cleanup signalled through the same absent process.kill and would have left 2. Recording the defect was not fixing the measurement; the measurement is now taken. WHAT THIS CLOSURE DOES NOT DO: it does not reinstate the 2026-08-27 reading, which stays RETRACTED, and it does not revert any correction plans 08-11 through 08-14 made - a later favourable measurement on a repaired instrument does not make an earlier vacuous one informative. Scope: one provider, one platform, one Caido version, one machine. Full transcription and the discriminator argument are in 08-SPIKE.md section 'The patched-probe A1 re-run and the pre-fix Control - TAKEN 2026-08-31'. | fixed |  | 2026-08-28T09:25:02.483Z | 2026-08-31T09:08:57.178Z |
...
| 24 | 08 | deviation | packages/backend/src/index.ts |  | reasonCode reports 'completed' for a SIGTERM death, so the machine-readable field cannot distinguish a cancelled turn from a completed one. MEASURED 2026-08-31 by plan 08-17 on the maintainer's macOS Caido 0.58.2 (darwin 25.6.0, provider claude-cli): a turn on which Stop WAS clicked produced state 'stopped', reason 'Provider exited with code 143.', reasonCode 'completed', exitCode 143. 143 = 128 + 15 = SIGTERM. The human-readable reason text and exitCode both carry the truth; reasonCode does not. WHY THIS IS NOT COSMETIC: reasonCode exists to be branched on, and the same session produced a SECOND run - discarded, recorded in 08-SPIKE.md as Reading D - whose turn ended on its own with reason 'Last provider turn completed. Send another message to continue.', reasonCode 'completed' and NO exitCode. The two runs are indistinguishable on reasonCode alone. Both sampled 0 to 1 to 0, so a consumer separating cancellation from normal completion by reasonCode would have filed a normally-finished turn as a cancellation measurement - which is exactly the substitution plan 08-17 had to make by hand, using the presence of exitCode as the discriminator, in order to keep the pre-fix Control honest. WHAT WOULD CLOSE THIS: a reasonCode value that distinguishes signal-terminated exits from natural completion, plus an executed assertion over the mapping. Scope: observed on one provider, one platform, one Caido version, on the pre-fix probe build 68199fa plus a1-probe-fix.patch; the field is not build-specific, so HEAD is expected to behave the same, but that expectation is NOT a reading and is not recorded as one. | open |  | 2026-08-31T09:10:21.909Z |  |
...
[
  ...
  },
  {
    ...
    "line": null,
    "description": "Exactly two native win32 kill-tree cases form the 2/2 contract. CI run 33599694679 on candidate 9aa22bbd4c26fecabc17745438a2217a24b31d3e executed 2 passed, 0 pending and 0 failed; the behavioural case killed the live parent and grandchild, and the resolution case used the absolute system-root taskkill.exe. Commit e1ac837 removed the unsafe already-exited/dead-pid exit-code and stderr vehicle; no reserved block exists and that datum remains accepted and deferred. LIF-01 is closed at the owned live-tree contract; AR-01 and AR-04 remain accepted residuals.",
    "status": "fixed",
    "reason": "CI https://github.com/six2dez/drift/actions/runs/33599694679",
    "recorded_at": "2026-08-24T15:54:03.743Z",
    "resolved_at": "2026-09-02T06:39:13Z"
  },
  {
    "id": 13,
    "kind": "unrun-verify",
  ...
  },
  {
    ...
    "file": "packages/backend/src/index.ts",
    "line": null,
    "description": "reapMcpOrphans / reapSessionOrphansIfIdle wiring is asserted only by source text. NARROWED 2026-08-28 by plan 08-16 and NOT CLOSED; the superseded text is preserved verbatim in the marked-correction note at the end of this file. The SECOND half of the original entry — whether Caido's LLRT sandbox can spawn pgrep at all — is now MEASURED and answered YES, by bare name, on the maintainer's macOS Caido 0.58.2 (build 39876b5); see entry 14 for the reading and its derivation. Neither reap therefore degrades silently to the enumerator-unavailable no-op on that platform. NARROWED AGAIN 2026-08-31 by plan 08-17 and STILL NOT CLOSED. The 2026-08-31 HEAD-build reading 'kind=reap exit=0 killed=2 ageMs=31' shows the reap reaching its signalling arm rather than only its no-match arm, so the mechanism is demonstrably capable of acting and not merely of running. WHAT KEEPS THIS ENTRY OPEN, unchanged by either reading: the wiring is still asserted only by source text, index.ts is still not importable under vitest, and the IDLE-GATED session reap reapSessionOrphansIfIdle was NOT exercised by the 2026-08-28 or the 2026-08-31 reading - both came from paths other than that call site, and the 2026-08-31 value is consistent with the START-UP previous-run reap, which is a third call site again. activeSessions was 0 in every capture, so the idle gate's interesting case (activeSessions greater than 0 with a Drift-owned MCP call in flight) has still never been observed. An entry narrowed is an entry still open.",
    "status": "open",
    "reason": "",
  ...
  },
  {
    ...
    "file": "packages/backend/src/index.ts",
    "line": null,
    "description": "Fourteenth vacuous gate in Phase 8, and the first that shipped as a READING rather than as a green tick. The A1 probe at the recoverable commit 68199fa (index.ts:5061) decided the verdict with 'const alive = signalRef.process?.kill?.(grandchildPid, 0) ?? false', an optional call on a primitive the SAME diagnostics run measured ABSENT five lines earlier ('spikeProcessKillType: undefined'); the absent case was coalesced to 'not alive', so the favourable string 'grandchild-died (detached honoured)' was emitted UNCONDITIONALLY. THE RED INPUT DID NOT EXIST. WHAT MAKES THIS INSTANCE DIFFERENT FROM THE TWELVE ALREADY RECORDED (entries 16, 17, 18, 19 and the self-reports in this phase's SUMMARYs): those produced a green tick, which a reviewer distrusts by habit; this one produced a plausible-looking hardware READING, which reviewers propagate. It propagated into eleven carriers, .planning/ROADMAP.md, .planning/REQUIREMENTS.md, four phase documents, three source files and this ledger's own entry 11, and was pinned there by verdict-gate.sh ARM B before anybody re-derived it. FIXED IN CODE: classifyLivenessObservation in packages/backend/src/kill-plan.ts (plan 08-12) is three-valued - alive / dead / inconclusive - and reports inconclusive when the liveness primitive is unavailable instead of coalescing it into a verdict. CLOSED 2026-08-31 by plan 08-17 on the exact reading this entry named as its closing condition. The A1 re-run was taken on the maintainer's macOS Caido 0.58.2 (darwin 25.6.0, provider claude-cli) on the probe build 68199fa plus a1-probe-fix.patch, plugin id d8aee773-b939-4164-a576-9c276ee30df8. spikeDetachedGroupKill read 'grandchild-died (detached honoured)', which is formatSpikeVerdict's dead arm and one of exactly three permitted values. THE INSTRUMENT COULD HAVE SAID OTHERWISE, which is the property the withdrawn reading lacked: classifyLivenessObservation DEFAULTS to inconclusive and must clear five gates before it can return dead (a spawned enumerator, no timeout, a usable pid, a numeric exit code that is neither undefined nor null, and that code being one of two recognised values), and the patch passes spawnThrew: !probe.spawned specifically so that spawnAndWait's SYNTHETIC code 1 on an unspawnable enumerator - byte-identical to ps's documented no-process-matched - cannot become the answer that the grandchild is gone. That is gap 1 rebuilt out of a different operator, and the patch refuses it by name. NOTE THAT spikeProcessKillType STILL READ 'undefined' in the same dump, reproducing the retracted reading's exact surface signature; what makes this one a measurement is that THE CODE BETWEEN THE TWO VALUES IS DIFFERENT, and the fixture check pgrep -f 'node -e' returning 0 immediately after the diagnostics call proves the PATCHED build is what ran, because the original's cleanup signalled through the same absent process.kill and would have left 2. Recording the defect was not fixing the measurement; the measurement is now taken. WHAT THIS CLOSURE DOES NOT DO: it does not reinstate the 2026-08-27 reading, which stays RETRACTED, and it does not revert any correction plans 08-11 through 08-14 made - a later favourable measurement on a repaired instrument does not make an earlier vacuous one informative. Scope: one provider, one platform, one Caido version, one machine. Full transcription and the discriminator argument are in 08-SPIKE.md section 'The patched-probe A1 re-run and the pre-fix Control - TAKEN 2026-08-31'.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-08-28T09:25:02.483Z",
    "resolved_at": "2026-08-31T09:08:57.178Z"
  },
  {
    "id": 21,
    "kind": "deviation",
  ...
  },
  {
    ...
    "file": "packages/backend/src/index.ts",
    "line": null,
    "description": "reasonCode reports 'completed' for a SIGTERM death, so the machine-readable field cannot distinguish a cancelled turn from a completed one. MEASURED 2026-08-31 by plan 08-17 on the maintainer's macOS Caido 0.58.2 (darwin 25.6.0, provider claude-cli): a turn on which Stop WAS clicked produced state 'stopped', reason 'Provider exited with code 143.', reasonCode 'completed', exitCode 143. 143 = 128 + 15 = SIGTERM. The human-readable reason text and exitCode both carry the truth; reasonCode does not. WHY THIS IS NOT COSMETIC: reasonCode exists to be branched on, and the same session produced a SECOND run - discarded, recorded in 08-SPIKE.md as Reading D - whose turn ended on its own with reason 'Last provider turn completed. Send another message to continue.', reasonCode 'completed' and NO exitCode. The two runs are indistinguishable on reasonCode alone. Both sampled 0 to 1 to 0, so a consumer separating cancellation from normal completion by reasonCode would have filed a normally-finished turn as a cancellation measurement - which is exactly the substitution plan 08-17 had to make by hand, using the presence of exitCode as the discriminator, in order to keep the pre-fix Control honest. WHAT WOULD CLOSE THIS: a reasonCode value that distinguishes signal-terminated exits from natural completion, plus an executed assertion over the mapping. Scope: observed on one provider, one platform, one Caido version, on the pre-fix probe build 68199fa plus a1-probe-fix.patch; the field is not build-specific, so HEAD is expected to behave the same, but that expectation is NOT a reading and is not recorded as one.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-08-31T09:10:21.909Z",
    "resolved_at": null
  }
]
````
...
> **Entry 12, superseded 2026-08-31 by Plan 08-23 — preserved, not deleted.** Its former table and
> JSON description read: "The three win32 kill-tree cases have never executed: they are
> skipIf-gated and no windows-latest run exists for them yet. The reserved dead-pid exit-code block
> is empty and the run URL is unfilled." Commit `e1ac837` had already removed that third test because
> a raw pid can be recycled after the owned process exits. The current entry therefore records the
> safe two-case suite and leaves the missing datum accepted/deferred; entry 12 stays `open`.
