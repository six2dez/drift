...
- [x] **Phase 8: Process Lifecycle** - Process-tree death on Windows *and* POSIX so cancel/timeout leaves no token-bearing orphan (completed 2026-09-02)
...
**Success Criteria** (what must be TRUE):
  ...
  1. A platform-branched `killTree(proc)` terminates the whole process tree on Windows via a spawn of **`<SystemRoot>\System32\taskkill.exe`** — resolved by absolute path, reading `SystemRoot` **and** `SYSTEMROOT` (Windows' own lookup is case-insensitive; the Linux CI runner's object lookup is not), trailing separators stripped by hand — with `["/pid", "<n>", "/t", "/f"]` and `windowsVerbatimArguments: false`. The bare name `taskkill.exe` (`DEFAULT_TASKKILL`) is the **last resort**, used only when neither casing is set. Guarded against an undefined pid, and against `0`, negatives and non-integers, in `buildKillTreePlan` — the one place a test can reach the guard, `index.ts` being unimportable under vitest. *(**Amended 2026-08-24 during the Phase 8 code-review fix pass**, per `08-REVIEW.md` WR-04. The original wording named `spawn("taskkill", …)` — the BARE name — and the move away from it is a **security** decision, recorded as threat **T-08-03** (*Spoofing / EoP*): Windows resolves a bare name through a search order that includes the working directory the Caido plugin host chose, which on a security tester's machine is more likely than average to contain something. The shipped mechanism follows the `getWhichCommand` precedent Phase 6 established at `platform.ts:255-294`. SC-1 as originally written was **satisfied by the insecure form** and would not have flagged its reintroduction. Amended in place for the same reason SC-2 was, and stated there: the gate stops the code, the amendment stops the intent. Same precedent as Phase 5 SC-1 and Phase 7 SC-2, per 05-CONTEXT.md D-02.)*
  2. On POSIX the same guarantee holds — no orphaned token-bearing process survives a turn — and it is served by **TWO mechanisms**, each covering the case the other cannot. **(a) The process-group path**, for the case where the CLI's MCP child STAYS in the CLI's process group: that child — which carries `CAIDO_TOKEN` in its environment — is killed with the parent, via `detached: true` at the provider spawn **plus a spawned `kill` process-group signal** — `spawn("kill", ["-TERM", "--", "-<pid>"])`, then the same with `-KILL` — **in addition to** the single-pid SIGTERM→SIGKILL ladder, which is KEPT on POSIX rather than replaced (LIF-02); the runtime's own negative-pid spelling of that signal stays BANNED, per the 2026-08-24 note below. **(b) The argv-marker orphan reap**, for the case where it does NOT — which is the case actually measured on 2026-08-27 for the one provider anyone has measured: a scan for Drift's own session-unique temp-directory marker adjacent to `mcp-server.mjs` (`buildSessionOrphanScanPlan` → `parseOrphanScanPids`), followed by a **POSITIVE single-pid kill** (`buildOrphanKillPlan`), wired at start-up (`reapMcpOrphans`) and on the idle-gated cancel/close/timeout path (`reapSessionOrphansIfIdle`, gated by `shouldReapSessionOrphans`). Clause (b) is a **stronger identity than (a), not a workaround**: it names its target by the target's OWN COMMAND LINE, so it is independent of process groups **by construction**, and it is the only termination path in Drift that can reach an MCP child Drift did not spawn. **What (b) does NOT cover — stated inside the criterion so this amendment cannot be read as claiming more than the mechanism delivers:** it is **POSIX only**, with no equivalent command-line enumerator on Windows, so the foreign-parented orphan class is unreachable there (**AR-04**); it is **idle-gated**, firing when no session is active rather than continuously, so cancelling one session while another is live leaves that orphan to clause (a) until the last session closes (**AR-07**); and it **depends on the enumerator being spawnable inside Caido's sandbox**, which is now MEASURED on real Caido 0.58.2: the release candidate forced the marker arm to `kind=reap exit=0 attempted=1 ageMs=109`, killed the separately grouped fixture, and only then removed the runtime root. Unsupported/error outcomes still fail closed into a no-op that removes no pre-existing termination path; no Windows enumerator is claimed (AR-04).

     *(**Criterion body amended 2026-08-28 by plan 08-13 — the criterion now names the mechanism that actually serves the guarantee. This is the THIRD amendment to this one criterion, so its trigger is recorded rather than assumed.** The trigger is a MEASUREMENT: assumption **A6 was FALSIFIED** by direct `ps` observation on 2026-08-27 — codex pid **43921** sat in pgid **43752** while its own `mcp-server.mjs` child pid **44284** sat in pgid **44284**, its own group — so a group signal aimed at the CLI's group cannot reach the token-bearing child for that provider, and the single-pid rung kept per OQ-2 signals the CLI rather than the child. Maintainer **recorded decision GD-01** directed building a mechanism that does not depend on A6; plans 08-06/08-07 shipped it, and until now this criterion did not name it. **The guarantee SC-2 exists for is UNCHANGED**, and nothing in clause (a) is weakened — every detail of the group half survives verbatim, including the concrete argv spelling, the ban on the runtime's negative-pid form, and the fact that the single-pid rung is KEPT rather than replaced. What changed is that the criterion is now **harder to satisfy**: it demands a second, named mechanism, and it states three boundaries on that mechanism which a bare "the reap closes it" would not. Amended in place under the same precedent both notes below cite — Phase 5 SC-1 and Phase 7 SC-2, per `05-CONTEXT.md` D-02. **The override path was considered and NOT taken:** `08-VERIFICATION.md` § *Override suggestion* carries verbatim the frontmatter that would accept this deviation instead of amending, and it remains available to the maintainer, who alone can supply its `accepted_by` and `accepted_at`.)*

     *(**Clause amended 2026-08-27 by plan 08-10 — the clause was STALE, the code was not relaxed.** The original wording read "instead of today's single-pid SIGTERM→SIGKILL ladder that leaves it to the CLI's own cleanup". The shipped `killTree` fires the single-pid rung FIRST on POSIX and then the group spawn, keeping both — **recorded decision OQ-2** (`08-02-PLAN.md`), pinned by a `proc.kill(` census in `index.source.test.ts` and by a source comment forbidding the rung's deletion. The code therefore does **strictly more** than this criterion asked, which is why the correction is to the criterion and not to the code. Amended in place for the same reason SC-1 and SC-2 were amended before, and stated there: the gate stops the code, the amendment stops the intent. **What CHANGED the rung's justification — CORRECTED 2026-08-28 by plan 08-13, because the argument written here on 2026-08-27 rests on a reading that has since been RETRACTED.** A1's 2026-08-27 reading was withdrawn on 2026-08-28: the probe at `68199fa` determined liveness with `signalRef.process?.kill?.(pid, 0) ?? false` on a runtime where the SAME diagnostics run measured `process.kill` ABSENT, so the optional chain yielded `undefined`, `?? false` made `alive === false`, and the favourable string was emitted UNCONDITIONALLY — the red input did not exist, so the reading carries no information. **A1 is OPEN, not measured**, and with A1 open the single-pid rung is defence against the **PRESENT** Caido as well as a future one. Note the DIRECTION of the 2026-08-27 error: it **UNDER-claimed** the rung's necessity, which is an error toward more caution — nothing shipped wrong on the strength of it and no code changes as a result. The rest of this note stands: the original "instead of today's single-pid ladder" clause really was stale, and correcting the criterion rather than the code really was the right call. The superseded argument is preserved verbatim beneath this note.)*
...
**Plans**: 23 plans total — 23 executed; Phase 8 closed 2026-09-02 at `08-VERIFICATION-6.md` (20/20)

**Closure evidence:** native Windows CI run `33599694679` executed the safe kill-tree contract 2/2
on candidate `9aa22bb`; a separately grouped marker fixture in the installed Caido build reached
`kind=reap exit=0 attempted=1 ageMs=109`, died, and was reaped before recursive runtime-root removal.
AR-01, AR-04, and the other accepted security residuals remain explicit boundaries, not hidden
extensions of LIF-01/LIF-02.
...
- [x] 08-04-PLAN.md — Windows evidence: the win32-gated integration suite, the recorded `taskkill` exit code, and a `--reporter=json` CI gate with anchors distinct from Phase 7's (LIF-01)
...
*Gap closure (added 2026-08-27 from `08-UAT.md`; five gaps plus three verifier advisories). **Corrected 2026-08-28 by plan 08-13 — the A1 half of the sentence below rested on a reading that has since been RETRACTED.** What the 2026-08-27 UAT round ACTUALLY established, and what it did not: **A6 was FALSIFIED** by direct `ps` observation (codex pid **43921** in pgid **43752**; its own `mcp-server.mjs` child pid **44284** in pgid **44284**), and a **token-bearing MCP orphan Drift never spawned** was found alive with `activeSessions: 0` (gap G-04). Both are measurements and NEITHER is withdrawn. **A1's reading is WITHDRAWN** — the probe determined liveness with `process.kill` on a runtime the same diagnostics run measured as not having it, so the favourable string was emitted UNCONDITIONALLY; **A1 is OPEN, not measured**. Maintainer decisions **GD-01** (build a mechanism that does not depend on A6) and **GD-02** (G-04 is fixed in Phase 8, not deferred) govern this set and are **UNAFFECTED by the retraction** — both were made on the A6 and G-04 findings, neither of which is withdrawn. Stated explicitly so a reader does not have to wonder.*

> *SUPERSEDED (written 2026-08-27; the A1 half was RETRACTED 2026-08-28):* Gap closure (added 2026-08-27 from `08-UAT.md`; five gaps plus three verifier advisories). UAT measured **A1 closed favourably** and **A6 FALSIFIED**, and found a token-bearing MCP orphan Drift never spawned. Maintainer decisions **GD-01** (build a mechanism that does not depend on A6) and **GD-02** (G-04 is fixed in Phase 8, not deferred) govern this set.
...
- [x] 08-07-PLAN.md — Expand to the remaining orphan classes: the start-up previous-run reap that closes AR-02/OQ-3, the idle-gated cancel/close/timeout reap that closes LIF-02 without A6, and six source gates (LIF-02, LIF-01)

**Gap Wave 3** *(blocked on Gap Wave 2)*

- [x] 08-08-PLAN.md — G-01: a Windows system root that does not come from the empty sandbox environment, applied to `taskkill.exe`, `where.exe` and `cmd.exe` (Phase 7's identical hole), with the scope boundary written into the source (LIF-01, LIF-02)
...
*Second gap-closure round (added 2026-08-28 from `08-VERIFICATION.md`; three failed gaps, one `insufficient_spec` item, five open `human_verification` items). The verdict: **the code is a net improvement; the record is a regression.** No code blockers — nothing stubbed, orphaned, hollow or unwired. Gap 1 is a record defect: assumption **A1's verdict was recorded as favourable on a reading the probe could not have produced any other way** — it determined liveness with `process.kill`, which the same diagnostics run measured absent, and coalesced the absent case into the favourable verdict. That is the phase's thirteenth vacuous gate and the first that produced a reading rather than a green tick, which is why it propagated into eleven carriers, the ROADMAP, REQUIREMENTS.md and ledger entry 11 — and why `verdict-gate.sh` now enforces it in both arms and goes red on the correction itself. Gaps 2 and 3 are criteria that have drifted from what measurement shows. The Windows execution leg stays deferred to Phase 9 SC-4. **(Wording corrected 2026-08-28 by plan 08-13: this sentence quoted the withdrawn verdict phrase on a LIVE line, which `verdict-gate.sh` ARM A/A1-STALE cannot distinguish from an assertion — correctly, because no line-oriented scan can. The claim is unchanged and nothing here is softened; only the spelling of the quoted phrase moved into the preserved quote below.)***

> *SUPERSEDED SPELLING (written 2026-08-28, corrected the same day):* Second gap-closure round (added 2026-08-28 from `08-VERIFICATION.md`; three failed gaps, one `insufficient_spec` item, five open `human_verification` items). The verdict: **the code is a net improvement; the record is a regression.** No code blockers — nothing stubbed, orphaned, hollow or unwired. Gap 1 is a record defect: assumption **A1 was recorded as CLOSED FAVOURABLY on a reading the probe could not have produced any other way** — it determined liveness with `process.kill`, which the same diagnostics run measured absent, and coalesced the absent case into the favourable verdict. That is the phase's thirteenth vacuous gate and the first that produced a reading rather than a green tick, which is why it propagated into eleven carriers, the ROADMAP, REQUIREMENTS.md and ledger entry 11 — and why `verdict-gate.sh` now enforces it in both arms and goes red on the correction itself. Gaps 2 and 3 are criteria that have drifted from what measurement shows. The Windows execution leg stays deferred to Phase 9 SC-4.
...
- [x] 08-15-PLAN.md — UD-01's resolved deliverable: a `lastOrphanReap` diagnostics key that says whether the orphan reap ran, was refused, or is inert — plus `a1-probe-fix.patch` and `verify-a1-patch.sh`, a committed probe fix proven to apply, type-check and build — **the gate goes green here**, and its red input is re-demonstrated at green (LIF-02)
...
- [x] 08-18-PLAN.md — Propagate the repaired 2026-08-31 A1 verdict across all live carriers and pointers while preserving the invalid 2026-08-27 reading as a marked retraction; harden `verdict-gate.sh` and pin the historical record class (LIF-01, LIF-02)
...
- [x] 08-20-PLAN.md — Replace path-equality retirement inference with explicit provider-start identity and execute the invalidated-lease/equal-pointer/root-recreation interleaving (LIF-01, LIF-02)

**Gap Wave 14** *(blocked on Gap Wave 13; shares `index.ts` and its source gate)*

- [x] 08-21-PLAN.md — Own Claude/Copilot configs before write, atomically publish them, and retain ownership across unlink failure with real-filesystem tests (LIF-01, LIF-02)

**Gap Wave 15** *(blocked on Gap Wave 14; shares Start wiring and its source gate)*

- [x] 08-22-PLAN.md — Require readable regular runtime artifacts plus parseable context before reuse, with real-filesystem directory/unreadable/malformed cases (LIF-01, LIF-02)
...
| 8. Process Lifecycle | 23/23 | Human needed — runtime evidence remains |  |
