...
| **A6** | The provider CLIs do not themselves place their MCP child in a different process group. | § *Measured experiment*, the whole POSIX mechanism | **MEASURED 2026-08-27 — FALSIFIED.** Codex's `mcp-server.mjs` child (pid **44284**) sat in its own process group **44284**, while the CLI (pid **43921**) sat in group 43752. A group signal aimed at the CLI's group cannot reach the token-bearing child, so for that provider LIF-02 is **not** closed by process-group signalling. One provider, on an instance Drift did not spawn; **Claude Code remains unmeasured**. The mechanism that answers it is the argv-marker orphan reap (plans 08-06/08-07), which identifies its target by the target's own command line and is independent of process groups by construction. Re-run: `ps -eo pid,ppid,pgid,args \| grep -E 'mcp-server\.mjs\|--mcp-config' \| grep -v grep` during a live turn. *(Marked correction. The superseded rating read: "**Medium-high, and untested.** If Claude Code or Gemini calls `setsid()` on its MCP child, the group kill misses it and LIF-02 is not closed. No CLI binary is executed anywhere in this project's CI (07-VALIDATION § Vehicle caveat item 3). **Mitigation: the same Wave 1 spike can check this with one real turn plus a `ps` read.** Strongly recommend the planner schedule it." — that mitigation named the `comm` specifier, which is the G-03 defect and could never have produced the reading.)* |
...
- [ ] **Spike (strongly recommended):** a temporary diagnostics path that reports, from a **real Caido install on macOS/Linux**, (a) `typeof process.kill`, (b) that a `detached: true` spawn's group kill reaches a grandchild, and (c) the `pgid` of a real CLI's MCP child (`ps -eo pid,ppid,pgid,args | grep -E 'mcp-server\.mjs|--mcp-config' | grep -v grep`). This is the **only** available closure for assumptions **A1** and **A6**, the two highest-risk items in this document, and it runs on hardware the maintainer already owns. *(**Command CORRECTED IN PLACE 2026-08-27**, UAT gap G-03. This document is a **living reference** — its § Validation Architecture is inherited by Phases 9 and 10 — and a document that will be **followed** must be correct, so the spelling is fixed here rather than annotated as superseded. The superseded form used the `comm` output specifier, which prints the executable **name**: the MCP server runs as `node <tmp>/mcp-server.mjs`, so its `comm` is `node` and a `mcp-server` alternation could never match it, under any circumstances, including one where A6 holds perfectly. It survived research → plan → plan-check → execution → verification because **no gate ever executed it**. Canonical procedure: `08-SPIKE.md` § *Step 3*.)*
