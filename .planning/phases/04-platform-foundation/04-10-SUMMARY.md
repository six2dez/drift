...
| 7 | `resolveCommand`'s `out` (`:1215` decl, `:1235` append) | `appendBounded` / **reuses** `SPAWN_STDOUT_MAX_CHARS` / head | **done (this plan)** |
...
| **T1** diagnostics data survives | `grep -c "lastNodeSearchCandidates" …` | `3` (decl `:172`, assign `:2019`, read `:3457`) |
| **T1** two cache call sites | `grep -c "resolveWithCache(resolutionCache" …` | `2` (bare token `3`, incl. the import) |
| **T1** both node consumers cached | `grep -c "await getCachedNodeExecutable()" …` | `2` — `:2049` (`requireNodeExecutable`), `:3410` (`getDiagnostics`) |
| **T1** one node key literal | `grep -c '{ key: "node"' …` | `1` — `:2040` |
...
| **T1** short-circuit before the cache | `grep -n "path.isAbsolute(command)\|resolveWithCache(resolutionCache, {"` | `:1206` **<** `:1214` |
...
| **T1** diagnostics still resolves node | `grep -n "nodeExecutable: nodeExecutable" …` | `:3456`, `\|\| "not found"`, fed by the `node` key |
...
| **T2** diagnostics keys | `grep -n "resolutionCache: describeResolutionCache\|resolutionCacheTtls:"` | `:3499` / `:3502`, TTLs rendered from `RESOLUTION_POSITIVE_TTL_MS` / `RESOLUTION_NEGATIVE_TTL_MS` |
...
- **The `path.isAbsolute` short-circuit stays outside the cache, and the reason is written at it.** It is one `fileExists` call, not worth an entry, and caching it would let a deleted or replaced absolute path linger for the whole 5-minute positive TTL (T-04-25).
...
- **T-04-25 (Tampering, absolute-path resolution being cached) — mitigated.** The `path.isAbsolute` short-circuit is evaluated at `:1206`, before the cache lookup at `:1214`, so a deleted or replaced absolute path is re-checked on every call rather than lingering for the positive TTL. The reason is commented at the branch.
...
- **Line numbers in every earlier Phase 4 summary are now stale for `index.ts`** — it has taken +450/-20 (04-08), +181/-41 (04-09) and +186/-43 (this plan). Re-locate by content, never by number. The current anchors: `resolveCommand` `:1202`, its `out` declaration `:1215` and append `:1235`, `checkProvider` `:1294`, `updateSettings`' merge `:1375` and invalidation `:1397-1403`, `getProviderStatuses` `:1526`, `checkProviderAvailability` `:1530-1539`, `getNodeExecutable` `:1997`, `getCachedNodeExecutable` `:2039-2041`, `requireNodeExecutable` `:2043-2051`, `getDiagnostics`' node read `:3410` and its two new keys `:3499`/`:3502`, the init seed `:3679-3680`.
